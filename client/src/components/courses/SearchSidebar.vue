...
import { ALL_DAYS, DAY_LABELS, GROUP_LABELS, type InSISDay, type InSISStudyPlanCourseGroup } from '@client/types/courses'
...
watch(
	() => filter.value.day,
	(val) => {
		selectedDays.value = val ? (Array.isArray(val) ? val : [val]) : []
	},
	{ immediate: true },
)
...
function handleSearch() {
	if (searchText.value) {
		courseSearch.setFilter('search', searchText.value)
	} else {
		courseSearch.clearFilter('search')
	}
	courseSearch.search()
}
...
function isDaySelected(day: InSISDay): boolean {
	const filterDays = filter.value.day
...
<template>
	<aside class="insis-search-panel">
		<h3 class="insis-panel-title">Parametry hledání</h3>

		<!-- Quick Search -->
		<div class="field-group">
			<label class="insis-label">Hledat</label>
			<div class="flex gap-2">
				<input v-model="searchText" type="text" class="insis-input flex-1" placeholder="Název nebo kód..." @keyup.enter="handleSearch" />
				<button class="insis-btn insis-btn-primary" :disabled="isLoading" @click="handleSearch">Hledat</button>
			</div>
		</div>

		<!-- Course Ident Filter -->
		<div class="field-group">
			<label class="insis-label">Kód předmětu</label>
			<input v-model="identFilter" type="text" class="insis-input" placeholder="např. 4IT101" @keyup.enter="handleIdentFilter" />
		</div>

		<!-- Day Filter -->
		<div class="field-group">
			<label class="insis-label">Dny v týdnu</label>
			<div class="flex flex-wrap">
				<button v-for="day in ALL_DAYS.slice(0, 5)" :key="day" class="insis-day-toggle" :class="{ active: isDaySelected(day) }" @click="toggleDay(day)">
		...
		<div v-if="isWizardComplete" class="field-group">
			<label class="insis-label">Typ předmětu</label>
		...
		<div v-if="!isWizardComplete && facets?.faculties?.length" class="field-group">
			<label class="insis-label">Fakulta</label>
			<select
				class="insis-select"
		...
		<div v-if="facets?.ects?.length" class="field-group">
			<label class="insis-label">Kredity (ECTS)</label>
			<select
				class="insis-select"
				:value="filter.ects || ''"
				@change="
		...
		<div v-if="hasActiveFilters" class="field-group mt-4 pt-4 border-t border-[var(--insis-border)]">
			<button class="insis-btn w-full" @click="clearAllFilters">Zrušit všechny filtry</button>
		</div>
	</aside>
</template>
