...
<template>
	...
	<div class="mb-4 grid grid-cols-3 gap-4">
		<div class="insis-card p-4 text-center">
			...
			<div class="mt-1 text-sm text-[var(--insis-text-2)]">Total Courses</div>
		</div>
		<div class="insis-card p-4 text-center">
			<div class="text-2xl font-bold text-[var(--insis-text)]">{{ props.database.totals.studyPlans }}</div>
			<div class="mt-1 text-sm text-[var(--insis-text-2)]">Study Plans</div>
		</div>
		<div class="insis-card p-4 text-center">
			<div class="text-2xl font-bold text-[var(--insis-text)]">{{ props.database.totals.faculties }}</div>
			<div class="mt-1 text-sm text-[var(--insis-text-2)]">Faculties</div>
		</div>
	</div>

	<!-- Faculty Freshness Table -->
	<div class="insis-card mb-4">
		<div class="border-b border-[var(--insis-border)] p-3 font-semibold text-[var(--insis-text)]">Faculty Freshness</div>
	...
	<!-- Stale Progress Bars -->
	<div class="insis-card mb-4">
		<div class="border-b border-[var(--insis-border)] p-3 font-semibold text-[var(--insis-text)]">Stale Courses</div>
		<div class="flex flex-col gap-3 p-4">
			<div v-for="stale in props.database.staleCourses" :key="stale.thresholdDays" class="flex items-center gap-3">
				<span class="w-32 shrink-0 text-sm text-[var(--insis-text-2)]">Stale &gt; {{ stale.thresholdDays }} days</span>
				<progress class="flex-1" :max="props.database.totals.courses" :value="stale.count" style="accent-color: var(--insis-danger)" />
				<span class="w-10 shrink-0 text-right text-sm text-[var(--insis-text-2)]">{{ stale.count }}</span>
			</div>
		</div>
	</div>
