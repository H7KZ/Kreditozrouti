...
### Key internals
- Controllers are **plain namespace objects** (not classes): `export const FooController = { async handleRequest(...) {} }`
...
- `VITE_*` env vars are baked into the build at compile time by Vite; runtime injection uses placeholder-swap in `docker-entrypoint.sh`
...
### Error handling
Jobs catch all exceptions internally, log them, and return `null`. BullMQ sees success; failed scrapes stay stale until the next scheduled run re-enqueues them. No automatic retry.
