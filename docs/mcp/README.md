...
| `MCP_ALLOWED_REDIRECT_HOSTS` | no       | `claude.ai,claude.com`     | Comma-separated https hosts (subdomains included) permitted as OAuth `redirect_uri`. HTTP loopback callbacks (`localhost`, `127.0.0.1`, `[::1]`, any port) are also permitted for installed clients such as Codex. Any other `redirect_uri` is rejected at registration and at authorize |
