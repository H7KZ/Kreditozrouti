...
4. Repeat for each unit type the course requires (some courses have only lectures; others require both a lecture and an exercise)
...
To unmark: expand the course (it's still visible if "Show completed courses" is on), and click **Mark as completed** again to toggle it off.
