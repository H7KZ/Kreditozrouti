...
API types consumed by the web app come from `@api/Contracts`, not from `@api/Database/types` directly.
...
The actual conflict detection logic lives in the shared package so it can be used by both web and API.
...
```vue
<script setup lang="ts">
import { useCoursesStore } from '@web/stores'
...
`faroModule.init(app, router)` is called from `../../web` after the Vue app is created:
