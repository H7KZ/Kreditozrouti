...
The web app is a Vue 3 SPA for browsing, filtering, and scheduling VŠE courses. Users configure a study plan through a
...
|-------------|----------------|------------------------------|
| `@web/*`    | `./src/*`      | Web source root              |
| `@api/*`    | `../api/src/*` | Type-only imports from API   |
| `@shared/*` | `../shared/*`  | Cross-package pure utilities |

**Rule:** Web may import types from `@api` (use `@api/Contracts`, not `@api/Database/types`) but must not import
runtime API code.
