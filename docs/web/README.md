...
- `apps/web/src/pages/` provides file-based routes: the wizard at `/`, the main app at `/courses`, public docs, and shared timetables at `/s/:id`.
- `apps/web/src/components/` contains the UI, including filters, courses, timetable, optimizer, wizard, docs, and common controls.
- `apps/web/src/stores/` owns application state. `apps/web/src/composables/` holds reusable Vue logic. `apps/web/src/utils/` holds small helpers.
- `apps/web/src/services/` calls the Axios client in `apps/web/src/api.ts`. Components call stores or composables rather than constructing API requests.
...
`apps/web/src/index.ts` creates the router, i18n, Pinia, and document head. It initializes optional Faro telemetry, hydrates saved schedules, timetable, UI, and wizard state, records a visit for the optional feedback prompt, then mounts the app. Course and wizard data are fetched when needed.
