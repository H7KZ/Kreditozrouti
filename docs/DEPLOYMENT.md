# Deployment — Index

> **Split documentation lives in [`docs/deployment/`](deployment/README.md).** This file is a navigation index.

| Document                                                         | Contents                                                         |
|------------------------------------------------------------------|------------------------------------------------------------------|
| [**deployment/README.md**](deployment/README.md)                 | Architecture diagram, environments, quick-start                  |
| [**deployment/DOCKER.md**](deployment/DOCKER.md)                 | Multi-stage builds, runtime env injection, GHCR registry         |
| [**deployment/CICD.md**](deployment/CICD.md)                     | GitHub Actions workflows, secrets, version directories, rollback |
| [**deployment/INFRASTRUCTURE.md**](deployment/INFRASTRUCTURE.md) | Traefik, GlitchTip, networking, volumes, env vars                |
| [**deployment/OPERATIONS.md**](deployment/OPERATIONS.md)         | Monitoring, security, backup, maintenance, troubleshooting       |
