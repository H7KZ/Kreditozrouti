...
import type { MCPCourse } from './CourseService.js'
import { getCoursesWithRelations } from './CourseService.js'
import type {
	ExploreResultDTO,
...
import { DEFAULT_WEIGHTS, MAX_EXPLORE_POOL_SIZE, MAX_POOL_SIZE, diversityFilter, scoreCandidate, solveWithDeadline } from '../domain/optimizer.js'
...
export async function optimize(db: Kysely<Database>, request: OptimizeRequest): Promise<OptimizeResponseDTO> {
	...
	if (request.mode === 'explore') {
		...
		const { courses } = await getCoursesWithRelations(db, { ids: allIds }, allIds.length, 0)
		return optimizeExplore(courses, courseIds, exploreIds, request, poolTruncated)
	}

	const { courses } = await getCoursesWithRelations(db, { ids: courseIds }, courseIds.length, 0)
