...
export default class OptimizerService {
      ...
      for (const unit of course.units ?? []) {
        for (const slot of unit.slots ?? []) {
          ...
          const candidate: SolverSlotCandidate = {
            unitId: unit.id, slotId: slot.id, courseId: course.id, unitType,
            day: slot.day ?? undefined, date: slot.date ?? undefined,
            timeFrom: slot.time_from, timeTo: slot.time_to, location: slot.location ?? undefined
          }
  ...
  private static snapshotAvailableTypes(course: MCPCourse): SelectedCourseUnitDTO['snapshotAvailableTypes'] {
    ...
    for (const unit of course.units ?? []) {
      for (const slot of unit.slots ?? []) {
        ...
        if (!seen.has(type)) { seen.add(type); types.push(type) }
      }
    }
    return types
  }
  ...
  private static rankAndMapCandidates(assignments: SolverAssignment[], constraints: SolverConstraints, courseById: Map<number, MCPCourse>): OptimizerCandidateDTO[] {
  ...
  private static runBuild(courses: MCPCourse[], request: OptimizeRequest, poolTruncated: boolean): OptimizeResponseDTO {
    ...
    const fullCandidates = OptimizerService.rankAndMapCandidates(OptimizerService.filterByCredit(raw1, request.constraints, courseById), request.constraints, courseById)
    ...
    for (const course of courses) {
      ...
      const ranked2 = OptimizerService.rankAndMapCandidates(OptimizerService.filterByCredit(raw2, request.constraints, courseById), request.constraints, courseById)
  ...
  private static runExplore(allCourses: MCPCourse[], basketIds: number[], exploreIds: number[], request: OptimizeRequest, poolTruncated: boolean): OptimizeResponseDTO {
    ...
    for (const course of exploreCourses) {
      ...
      exploreResults.push({
        course_id: course.id, course_ident: course.ident,
