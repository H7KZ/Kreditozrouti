...
import { useFiltersStore, useUIStore } from '@web/stores'
...
<template>
	...
	>
		...
		>
			...
			<span class="text-xs leading-none font-medium">{{ $t('pages.courses.courseList') }}</span>
		</button>
		...
		>
			...
			<span class="text-xs leading-none font-medium">{{ $t('pages.courses.myTimetable') }}</span>
		</button>
		...
		>
			<div class="relative">
				...
				<span v-if="filtersStore.activeFilterCount > 0" class="absolute -top-1 -right-1 h-2 w-2 rounded-full bg-(--insis-blue)" aria-hidden="true" />
			</div>
			<span class="text-xs leading-none font-medium">{{ $t('common.filters') }}</span>
		</button>
	</nav>
</template>
