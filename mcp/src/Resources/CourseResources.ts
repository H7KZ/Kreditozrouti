...
export default class CourseResources {
	static register(server: McpServer, db: Kysely<Database>): void {
		...
		registerResourceTemplate(server, db, {
			name: 'VŠE Course',
			template,
			description: 'Full VŠE course detail by numeric ID, including units and time slots. Times are minutes from midnight (0–1439). URI: vse://course/{id}',
			mimeType: 'application/json',
			handler: async (uri, variables, db) => {
				const id = Number((variables as Record<string, string>)['id'])
				if (Number.isNaN(id)) {
					return {
