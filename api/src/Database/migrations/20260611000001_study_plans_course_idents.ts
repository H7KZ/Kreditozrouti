...
export async function up(db: Kysely<any>): Promise<void> {
    ...
    await db.schema
        ...
        .addColumn('study_plan_id', 'integer', col =>
            col.notNull().references(`${StudyPlanTable._table}.id`).onDelete('cascade')
        )
    ...
    await createUniqueIndexSafe(db, 'idx_spc_idents_unique', StudyPlanCourseIdentTable._table, [
        'study_plan_id',
        'course_ident'
    ])
    ...
    if (fkResult.rows.length > 0) {
        ...
        await sql.raw(
            `ALTER TABLE \`${StudyPlanCourseTable._table}\` DROP FOREIGN KEY \`${fkName}\``
        ).execute(db)
    }

    // 5. Make course_id NOT NULL
    await sql.raw(
        `ALTER TABLE \`${StudyPlanCourseTable._table}\` MODIFY course_id INT NOT NULL`
    ).execute(db)
...
export async function down(db: Kysely<any>): Promise<void> {
    ...
    await sql.raw(
        `ALTER TABLE \`${StudyPlanCourseTable._table}\` MODIFY course_id INT NULL`
    ).execute(db)
