import { EventTable, UserTable, UsersEvents } from '@api/Database/types';
...
export async function up(mysql: Kysely<any>): Promise<void> {
    await mysql.schema
        ...
        .addColumn('user_id', 'varchar(255)', col => col.notNull().references(`${UserTable._table}.id`).onDelete('cascade'))
