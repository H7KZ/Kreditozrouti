...
Divide non-trivial tasks into three phases:

1. **Plan** — read the relevant `docs/` files, map the affected files with `grep_search`
2. **Execute** — edit files atomically, one logical change at a time
3. **Verify** — run `make lint`, check the doc-review rule above

Never skip the Plan phase on tasks that touch multiple packages.
